...
Download these": 
## http://hgdownload.cse.ucsc.edu/gbdb/hg38/liftOver/hg38ToDasNov3.over.chain.gz
## https://hgdownload.cse.ucsc.edu/gbdb/mm10/liftOver/mm10ToDasNov3.over.chain.gz


## Human homologs 
library(EGAD)
f.x = attr.human[,1] == "chrX"
m = match( attr$name, attr.human$name[f.x])
f.arm = !is.na(m)

scaff.freq = count(attr$scaffold[f.arm])
potentialXscaff = scaff.freq[scaff.freq[,2] >  9 ,1]
scaff.freq.all = count(attr$scaffold) 
...
## Mouse genomic matches 
chain = read.table("chrX.chainMm10.txt")
scaff.sizes = unique(chain[,3:4]  )
