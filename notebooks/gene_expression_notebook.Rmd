...
```{r}
source("Z:/useful4.r")
load("U:/armadillo/updated/cpm.strand.comb.Rdata")
load("U:/armadillo/updated/counts.strand.comb.Rdata")
load("U:/armadillo/updated/gene_annotations_v0.95_mod.Rdata")
load("U:/armadillo//armadillo.helper.Rdata")
load("U:/armadillo/updated/armadillo_hem.Rdata")
load("U:/armadillo/updated/ref.strand.test_train.Rdata")
# Variables 
N = dim(X.cpm.all)[1]
n_quads = 5
n_times = 3 
...
load(file=paste0("U:/armadillo/updated/anova.quad",j,".Rdata")) 
...
for(j in 1:n_quads){ 
  ...
  load(paste0("U:/armadillo/updated/anova.quad",j,".Rdata"))
...
# Plot
```{r}
load(paste0("U:/armadilo/updated/random.sets.", dataset, ".Rdata") )
...
pdf("U:/armadillo/updated/heatmaps.modules.pdf")
for(j in 1:n_quads){ 
...
```{r}
pdf("U:/armadillo/updated/perfect.pred.prop.pdf")
...
```{r}
load(file="U:/armadillo/updated/null.perfect.pred.Rdata")
...
```{r, eval = FALSE}
load("U:/armadillo/updated/model.pred.cds.Rdata")
...
```{r, eval = FALSE}
load("U:/armadillo/updated/distr.model.comb.Rdata")
load("U:/armadillo/updated/models/model.means.shuff.Rdata")
...
## C: ASE feature genes 
```{r}
load("U:/armadillo/updated/ase.feature.genes.Rdata")
...
for(j in 1:n_quads){
load(file=paste("U:/armadillo/coexp/rocs.perfect.pred.clust.coexp.quad.loo",j,"Rdata", sep=".") ) 
...
for(j in 1:n_quads){
load(file=paste("U:/armadillo/coexp/rocs.perfect.pred.clust.coexp.quad",j,"Rdata", sep=".") ) 
...
## GO for armadillos
```{r}
load("Y:/human/GO/March2019/GO.human.Rdata")
gosums = colSums(GO.human.nonIEA )
annot =  GO.human.nonIEA 

load("U:/armadillo/coexp/coexp.aggr.Rdata")
library(EGAD)
...
load("Y:/genome/gene_annotations_v29.Rdata")
load("~/sballouz/genome/gene_annotations_v29.Rdata")
attr.human = attr
load("Y:/armadillo/ensembl/gene_annotations_v0.95_mod.Rdata")
load("~/sballouz/armadillo/ensembl/gene_annotations_v0.95_mod.Rdata")
attr.arm = attr
...
```{r}
load("U:/armadillo/updated/task1_results_GOarm.mat.Rdata")
...
```{r}
load("U:/armadillo/coexp/aurocs.go1.blood.clust.Rdata") 
aurocs.go1.blood = aurocs.go1 
load("U:/armadillo/coexp/aurocs.go1.arm.clust.Rdata") 
...
load("U:/armadillo/coexp/annot.agg.Rdata") 
...
```{r}
frac = read.table("U:/armadillo/deconvol/CIBERSORTx_Job5_Adjusted.txt", header=T, sep="\t")
frac2 = t(frac[,2:7])
barplot(frac2[,pData$Quad==j], col=inferno(6)) 
colnames(frac2) = pData$ID
pdf("U:/armadillo/updated/cibersort.pdf")
for(j in 1:5) {
...
```{r}
load("U:/armadillo/data_from_old_papers/table1_scutes_newman1913.Rdata")
o = order(scutes[,2] ) 
pdf("U:/armadillo/data_from_old_papers/historicaldata.pdf")
...
load("U:/armadillo/data_from_old_papers/storrs_1968.Rdata")
